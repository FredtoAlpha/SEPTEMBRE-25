...
// Obtenir les élèves disponibles pour les groupes
function getStudentsForGroups() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const students = [];
    
    // Chercher d'abord dans les onglets INT (sauvegardés)
    const sheets = ss.getSheets();
    const intSheets = sheets.filter(sheet => sheet.getName().endsWith('INT'));
    
    if (intSheets.length > 0) {
      // Utiliser le dernier onglet INT
      ...
      if (data.length > 1) {
        ...
        const sexeCol = headers.indexOf('SEXE');
        const classeCol = headers.indexOf('CLASSE');
        const lv1Col = headers.indexOf('LV1');
        const comCol = headers.indexOf('COM');
        const traCol = headers.indexOf('TRA');
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          if (row[nomCol]) {
            students.push({
              id: `eleve${i}`,
              nom: row[nomCol] || '',
              prenom: row[prenomCol] || '',
              sexe: row[sexeCol] || '',
              classe: row[classeCol] || '',
              lv1: row[lv1Col] || '',
              com: parseFloat(row[comCol]) || 0,
              tra: parseFloat(row[traCol]) || 0,
              part: parseFloat(row[partCol]) || 0,
              scores: {} // À remplir si disponible
            });
          }
        }
      }
    } else {
      // Sinon, utiliser la feuille ELEVES
      const elevesSheet = ss.getSheetByName('ELEVES');
      if (elevesSheet) {
        const data = elevesSheet.getDataRange().getValues();
    ...
    if (scoresSheet) {
      ...
      if (scoresData.length > 1) {
        ...
        const idCol = headers.indexOf('ID');
        const mathsCol = headers.indexOf('MATHS');
        const francaisCol = headers.indexOf('FRANCAIS');
        ...
        for (let i = 1; i < scoresData.length; i++) {
          ...
          if (student) {
            student.scores = {
              M: parseFloat(row[mathsCol]) || 0,
              F: parseFloat(row[francaisCol]) || 0
            };
          }
        }
      }
...
// 1. PATCH: Obtenir les élèves avec leurs scores depuis les onglets INT
function getStudentsForGroups() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const students = [];
    ...
    intSheets.forEach(sheet => {
      const className = sheet.getName(); // ex: "6°1INT"
      const data = sheet.getDataRange().getValues();
      
      if (data.length > 1) {
        const headers = data[0];
        console.log(`📋 En-têtes pour ${className}:`, headers);
        
        // Indices des colonnes - CORRECTION : utiliser LV2 au lieu de LV1
        const indices = {
          nom: headers.indexOf('NOM'),
          prenom: headers.indexOf('PRENOM'),
          sexe: headers.indexOf('SEXE'),
          lv2: headers.indexOf('LV2'), // CORRECTION : LV2 au lieu de LV1
          com: headers.indexOf('COM'),
          tra: headers.indexOf('TRA'),
          part: headers.indexOf('PART'),
          scoreF: 20, // Colonne U (index 20)
          scoreM: 21  // Colonne V (index 21)
        };
        
        console.log(`🔍 Indices pour ${className}:`, indices);
        
        // Lire les élèves
        for (let i = 1; i < data.length; i++) {
          const row = data[i];
          if (row[indices.nom]) {
            const langueRaw = (row[indices.lv2] || '').toString().trim();
            const langue = langueRaw.toUpperCase();
            
            // Log pour debug
            console.log(`Élève ${row[indices.nom]} - Langue brute: "${langueRaw}" - Langue normalisée: "${langue}"`);
            
            let langueType = 'ESP'; // Par défaut ESP
            let langueBadge = 'ESP par défaut';
            ...
            if (langue === 'ITA') {
              ...
              langueType = 'ESP';
              langueBadge = 'ESP';
            } else if (!langue || langue === '') {
              // Explicitement ESP par défaut pour les valeurs vides
              langueType = 'ESP';
              langueBadge = 'ESP (par défaut)';
            } else {
              // Autres langues non reconnues → ESP par défaut
              langueType = 'ESP';
              langueBadge = `ESP par défaut (${langueRaw})`;
            }
            
            console.log(`👤 ${row[indices.nom]} ${row[indices.prenom]} - Langue: "${langueRaw}" → ${langueBadge}`);
            ...
            students.push({
              ...
              classe: nettoyerNomClasse(className.replace('INT', '')), // Nettoyer le nom de classe
              lv2: langueType, // Stocker le type de langue normalisé (colonne F = LV2)
              lv2Raw: langueRaw, // Stocker la valeur brute pour affichage
              lv2Badge: langueBadge, // Badge pour l'affichage
              com: parseFloat(row[indices.com]) || 0,
              tra: parseFloat(row[indices.tra]) || 0,
              part: parseFloat(row[indices.part]) || 0,
              scores: {
                F: parseInt(row[indices.scoreF]) || 0, // Score Français (1-4)
                M: parseInt(row[indices.scoreM]) || 0  // Score Maths (1-4)
              }
            });
          }
        }
    ...
    console.log(`✅ Total élèves chargés: ${students.length}`);
    console.log('📊 Répartition par langue:');
    const langues = {};
    students.forEach(s => {
      const lang = s.lv2 || 'VIDE';
      langues[lang] = (langues[lang] || 0) + 1;
    });
    console.log(langues);
...
// 2. PATCH: Obtenir les classes disponibles depuis les onglets INT
function getINTClasses() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheets = ss.getSheets();
    
    // Filtrer et transformer les noms des onglets INT
    const intClasses = sheets
      .map(sheet => sheet.getName())
      .filter(name => name.endsWith('INT'))
      .map(name => nettoyerNomClasse(name.replace('INT', ''))) // Nettoyer le nom de classe
      .sort();
    
    console.log('📋 Classes INT nettoyées:', intClasses);
    return intClasses;
  } catch (error) {
    console.error('Erreur getINTClasses:', error);
    return [];
  }
}
